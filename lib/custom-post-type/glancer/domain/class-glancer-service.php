...
/**
 * Class Nice_Team_GlancerService
 *
 * This class deals with typical operations over Nice_Team_Glancer instances.
 *
 * @since 1.1
 */
class Nice_Team_GlancerService extends Nice_Team_Service {
	/**
	 * Create and return a new Nice_Team_Glancer instance.
	 *
	 * @since  1.1
	 *
	...
	protected function setup( array $data ) {
		/**
		 * Obtain an instance of Nice_Team_Pointer_Collection.
		 */
	 ...
	 */
	public function add_item( Nice_Team_Glancer $glancer, $post_types, $statuses = 'publish', $glyph = '' ) {
		/**
		 * If relevant output action hook has already passed, then there's no
		 * point in proceeding.
		 */
		if ( did_action( 'dashboard_glance_items' ) ) {
			_doing_it_wrong( __CLASS__, esc_html__( 'Trying to add At a Glance items to dashboard widget after hook already fired', 'nice-team-plugin-textdomain' ), '1.1' );
			return;
		}
