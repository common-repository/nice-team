...
/**
 * Class Nice_Team_MetaboxCreateResponder
 *
 * This class takes charge of the interaction of created Nice_Team_Metabox
 * instances with WordPress APIs.
 *
 * @since 1.1
 */
class Nice_Team_MetaboxCreateResponder extends Nice_Team_CreateResponder {
	/**
	...
	protected function get_metabox_settings( $post_type ) {
		...
		$defaults = array(
			'id' 			=> $data->id ? : 'nice-team-' . $post_type . '-settings',
			'title' 		=> sprintf( esc_html__( '%s Settings', 'nice-team-plugin-textdomain' ), ucfirst( $post_type ) ),
			'callback' 		=> 'nice_team_metabox_print',
			'post_type'     => $post_type,
		...
		$settings = apply_filters( 'nice_team_metabox_' . $post_type . '_settings', $settings );
