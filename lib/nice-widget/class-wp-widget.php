 ...
 */
abstract class Nice_Team_WP_Widget extends WP_Widget {
	/**
	 ...
	 *
	 * @since 1.0
	 * @var   Nice_Team_Widget_Normalizer
	 */
	...
	public function __construct() {
		...
		$this->refresh           = apply_filters( 'nice_team_widget_refresh', $this->refresh );
		$this->normalizer        = new Nice_Team_Widget_Normalizer( $this->instance_defaults );
		$this->cache             = new Nice_Team_Widget_Cache( $this->id . ':' . $order, $this->refresh );
		$this->textdomain        = nice_team_textdomain();
		 ...
		 */
		do_action( 'nice_team_widget_loaded', $this );
	 ...
	 *
	 * @see   Nice_Team_Widget::normalizer::normalize_instance()
	 *
	 * @since 1.0
	 *
	 * @param $instance
	 ...
	 *
	 * @see   Nice_Team_Widget::cache::set()
	 *
	 * @since 1.0
	 *
	 ...
	 *
	 * @see   Nice_Team_Widget::cache::get()
	 *
	 * @since 1.0
	 *
	 * @return null|string
	 ...
	 * Cache contents of the widget.
	 *
	 * @see    Nice_Team_Widget::cache::flush_cache()
	 *
	 * @since  1.0
	 *
	 * @return null|string
	 ...
	 * @since  1.0
	 *
	 * @uses   nice_team_admin_get_template_part()
	 *
	...
	function form( $instance ) {
		 ...
		 * of the template.
		 */
		nice_team_admin_set_current_widget( $this );

		/**
		 * Call form template.
		 */
		nice_team_admin_get_template_part( str_replace( '.php', '', $this->form_template ) );

		/**
		 * Kill current widget.
		 */
		nice_team_admin_unset_current_widget();
	}
	...
	protected function print_widget() {
		 ...
		 */
		nice_team_set_current_widget( $this );
		 ...
		 */
		nice_team_get_template( $this->template );
		 ...
		 */
		nice_team_unset_current_widget();
	}
	 ...
	 * @since 1.0
	 *
	 * @param Nice_Team_WP_Widget $widget Instance of widget object.
	 */
	public static function widget_title( Nice_Team_WP_Widget $widget ) {
		$title = isset( $widget->args['title'] ) ? $widget->args['title'] : '';

		$widget_title = '<h3 class="widget-title">' . $title . '</h3>';

		echo apply_filters( 'nice_team_widget_title', $widget_title, $title, $widget ); // WPCS: XSS ok.
	}
}
